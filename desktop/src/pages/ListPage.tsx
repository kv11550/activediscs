...
const HashePage = (props: any) => {
    ...
    const download = () => {
        ...
        saveAs(blob, `List_${contextKey}_${selectItem.id}.txt`);
    }
        ...
        <div className="grid grid-cols-12 gap-2 flex-shrink-0 w-full">
            ...
            <div className="col-span-1 bg-white flex ">
                ...
                <ul className="bg-white rounded-lg border text-sm border-gray-200 w-full text-gray-900">
                    {
                        idList.map(item => (
                            <li className={`px-2 py-1 mb-1 border-b border-gray-200 w-full overflow-x-auto ${selectItem.id === item ? "bg-gray-200" : ''}  `} onClick={() => getValue(item)}>
                                {`${item}`}
                            </li>
                        ))
                    }
